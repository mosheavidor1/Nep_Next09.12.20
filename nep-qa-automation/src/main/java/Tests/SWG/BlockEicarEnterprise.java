...
public class BlockEicarEnterprise extends GenericTest {

    private CloudActions action;

    @Factory(dataProvider = "getData")
    public BlockEicarEnterprise(Object dataToSet) {
        super(dataToSet);
        action = new CloudActions();
    }

    @Test(alwaysRun = true)
    public void BlockEicarEnterprise () throws Exception {
    ...
    public void CheckSiteBlocked2 () throws IOException {
        action.LaunchApplication(data.get("Browser"),PropertiesFile.getCurrentClusterLB() );
        action.SetApplicationUrl(data.get("Block Site"));
        //action.CheckBlockedPage2();
